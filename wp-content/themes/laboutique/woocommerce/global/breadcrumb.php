...
$before='<li>';
$after='</li>';
$wrap_before='<ul class="pull-right breadcrumb">';
$wrap_after='</ul>';
...
if ( ( ! is_front_page() && ! ( is_post_type_archive() && get_option( 'page_on_front' ) == wc_get_page_id( 'shop' ) ) ) || is_paged() ) {
	...
	//if ( ! empty( $home ) ) {
		echo $before . '<a class="home" href="' . apply_filters( 'woocommerce_breadcrumb_home_url', home_url() ) . '">' . __("Home",DOMAIN) . '</a>' . $after . $delimiter;
	//}
